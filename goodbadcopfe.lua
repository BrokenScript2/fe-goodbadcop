...
local SKILLTEXT = CreateLabel(SKILLFRAME, "[FE GOOD AND BAD COP BY Broken!Script?#0082]", SKILLTEXTCOLOR, SKILLTEXTSIZE, SKILLFONT, 0, 2, 0, "Skill text")
...
function ApplyDamage(Humanoid,Damage)
	...
	if DEAD == true then
		if MODE == "GoodCop" then
			...
			local args = {
    [1] = "I'm simply laying down the law",
    [2] = "All"
}
...
game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(unpack(args))
		elseif MODE == "BadCop" then
			...
			local args = {
    [1] = "You should've taken account of the law.",
    [2] = "All"
}
...
function BatonStun()
	...
	local TOUCH = BATON.Touched:Connect(function(hit)
		if hit.Parent:FindFirstChildOfClass("Humanoid") then
			...
			if TORSO and HUM.Health > 100 then
				local PASS = true
				for i = 1, #HITS do
...
function TheRightToRemainSilent()
	...
	if TARGET ~= nil then
		if TARGET.Parent:FindFirstChildOfClass("Humanoid") then
			local HUM = TARGET.Parent:FindFirstChildOfClass("Humanoid")
			local ROOT = TARGET.Parent:FindFirstChild("HumanoidRootPart") or TARGET.Parent:FindFirstChild("Torso") or TARGET.Parent:FindFirstChild("UpperTorso")
			if ROOT and HUM.Health > 100 then
				local FOE = Mouse.Target.Parent
				ATTACK = true
				Rooted = true
				CharacterFade(C3(1,1,1),70)
				...
				ROOT.Anchored = true
				CreateSound(289556450, RootPart, 5, 1.4, false)
				Chatter("You remain silent and DEAD!",0)
				local args = {
    [1] = "You remain silent and DEAD!",
    [2] = "All"
}
